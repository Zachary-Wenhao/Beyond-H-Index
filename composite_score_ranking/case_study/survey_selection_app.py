...
def load_data():
    ...
    try:
        ...
        try:
            df = pd.read_csv('../data/nlp_researcher_metrics.csv')
            return df
        except FileNotFoundError:
            st.error("❌ Main dataset not found. Please ensure 'nlp_researcher_metrics.csv' exists in the parent directory or in '../data/'.")
            return None
